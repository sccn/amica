...
! #endif
! include 'mkl_vml.f90'
!(07/17/2025) JS, updating to use newer compilers
...
seed_array = c1 * (myrank+1) * (/(i, i=1,seed_size)/)
call random_seed(PUT=seed_array)
deallocate(seed_array)
! (07/17/2025) JS, updating to modern compiler with newer openmpi + gcc + mkl
...
if (do_newton) then
   allocate( dbaralpha_numer_tmp_t (num_mix,nw,num_models,num_thrds),stat=ierr); &
call tststat(ierr); dbaralpha_numer_tmp_t = dble(0.0)
   allocate( dbaralpha_numer_tmp (num_mix,nw,num_models),stat=ierr); &
call tststat(ierr); dbaralpha_numer_tmp = dble(0.0)
   allocate( dbaralpha_numer (num_mix,nw,num_models),stat=ierr); &
call tststat(ierr); dbaralpha_numer = dble(0.0)
   allocate( dbaralpha_denom_tmp_t (num_mix,nw,num_models,num_thrds),stat=ierr); &
call tststat(ierr); dbaralpha_denom_tmp_t = dble(0.0)
   allocate( dbaralpha_denom_tmp (num_mix,nw,num_models),stat=ierr); &
call tststat(ierr); dbaralpha_denom_tmp = dble(0.0)
   allocate( dbaralpha_denom (num_mix,nw,num_models),stat=ierr); &
call tststat(ierr); dbaralpha_denom = dble(0.0)
   allocate( dlambda_numer_tmp_t (num_mix,nw,num_models,num_thrds),stat=ierr); &
call tststat(ierr); dlambda_numer_tmp_t = dble(0.0)
   allocate( dlambda_numer_tmp (num_mix,nw,num_models),stat=ierr); &
call tststat(ierr); dlambda_numer_tmp = dble(0.0)
   allocate( dlambda_numer (num_mix,nw,num_models),stat=ierr); &
call tststat(ierr); dlambda_numer = dble(0.0)
   allocate( dlambda_denom_tmp_t (num_mix,nw,num_models,num_thrds),stat=ierr); &
call tststat(ierr); dlambda_denom_tmp_t = dble(0.0)
   allocate( dlambda_denom_tmp (num_mix,nw,num_models),stat=ierr); &
call tststat(ierr); dlambda_denom_tmp = dble(0.0)
   allocate( dlambda_denom (num_mix,nw,num_models),stat=ierr); &
call tststat(ierr); dlambda_denom = dble(0.0)
   allocate( dkappa_numer_tmp_t (num_mix,nw,num_models,num_thrds),stat=ierr); call &
       tststat(ierr); dkappa_numer_tmp_t = dble(0.0)
   allocate( dkappa_numer_tmp (num_mix,nw,num_models),stat=ierr); call &
       tststat(ierr); dkappa_numer_tmp = dble(0.0)
   allocate( dkappa_numer (num_mix,nw,num_models),stat=ierr); call &
       tststat(ierr); dkappa_numer = dble(0.0)
   allocate( dkappa_denom_tmp_t (num_mix,nw,num_models,num_thrds),stat=ierr); call &
       tststat(ierr); dkappa_denom_tmp_t = dble(0.0)
   allocate( dkappa_denom_tmp (num_mix,nw,num_models),stat=ierr); call &
       tststat(ierr); dkappa_denom_tmp = dble(0.0)
   allocate( dkappa_denom (num_mix,nw,num_models),stat=ierr); call &
       tststat(ierr); dkappa_denom = dble(0.0)
   allocate( dsigma2_numer_tmp_t (nw,num_models,num_thrds),stat=ierr); call &
       tststat(ierr); dsigma2_numer_tmp_t = dble(0.0)
   allocate( dsigma2_numer_tmp (nw,num_models),stat=ierr); call &
       tststat(ierr); dsigma2_numer_tmp = dble(0.0)
   allocate( dsigma2_numer (nw,num_models),stat=ierr); call &
       tststat(ierr); dsigma2_numer = dble(0.0)
   allocate( dsigma2_denom_tmp_t (nw,num_models,num_thrds),stat=ierr); call &
       tststat(ierr); dsigma2_denom_tmp_t = dble(0.0)
   allocate( dsigma2_denom_tmp (nw,num_models),stat=ierr); call &
       tststat(ierr); dsigma2_denom_tmp = dble(0.0)
   allocate( dsigma2_denom (nw,num_models),stat=ierr); call &
       tststat(ierr); dsigma2_denom  = dble(0.0)
end if
...
do
   ...
   if (seg_rank == 0) then
      ...
      if (mod(iter,outstep) == 0) then
         ...
         write(20,"(a,i6,a,f13.10,a,f14.10,a,f13.10,a,2(e13.5),a,f6.2,a,f5.1,a)") &
 ' iter', iter,' lrate = ', lrate,  ' LL = ', LL(iter), &
              ' nd = ', ndtmpsum,', D = ', maxval(Dsum), minval(Dsum), &
              '  (', t0, ' s, ', dble(max_iter-iter)*t0/dble(3600), ' h)'
         call flush(6)
         ...
         if (use_min_dll) then
             if ((LL(iter) - LL(iter-1)) < min_dll) then
                 ...
                 if (numincs > maxincs) then
                    leave = .true.
                    print *, 'Exiting because likelihood increasing by less than ', &
 min_dll, ' for more than ', maxincs, ' iterations ...'
                    write(20,"(a,e13.5,a,i6,a)") 'Exiting because likelihood increasing by less than ', &
 min_dll, ' for more than ', maxincs, &
                         ' iterations ...'
                end if
             else
                numincs = 0
   ...
   if (startover) then
      ...
      !----- reject data
      if ((do_reject .and. (maxrej > 0)) .and. ((iter==rejstart) .or. &
((mod(max(1,iter-rejstart),rejint) == 0) .and. (numrej < maxrej)))) then
   ...
   do hh = h+1,num_models
      do i = 1,nw
         do ii = 1,nw
            ...
            t0 = abs(sum(tmpvec3*A(:,comp_list(ii,hh)))) / &
(sqrt(sum(tmpvec3*A(:,comp_list(i,h))))*sqrt(sum(tmpvec4*A(:,comp_list(ii,hh)))))
...
nx,Wtmp,nw,dble(1.0),xtmp(bstrt:bstp,:),tblksize)
...
subroutine write_history
  if (myrank == 0) then
    ...
    open(unit=29,file=trim(outdirparam)//'/history/'//trim(adjustl(tmpstring))//'/' &
      //outfile_c,access='direct',status='replace',recl=2*nbyte*nw*num_models)
    write(29,rec=1) c; call flush(29); close(29)
    open(unit=9,file=trim(outdirparam)//'/history/'//trim(adjustl(tmpstring))//'/' &
      //outfile_w,access='direct',status='replace',recl=2*nbyte*nw*nw*num_models)
    write(9,rec=1) W; call flush(9); close(9)
    open(unit=10,file=trim(outdirparam)//'/history/'//trim(adjustl(tmpstring))//'/' &
      //outfile_gamma,access='direct',status='replace',recl=2*nbyte*num_models)
    write(10,rec=1) gm; call flush(10); close(10)
    open(unit=11,file=trim(outdirparam)//'/history/'//trim(adjustl(tmpstring))//'/' &
      //outfile_alpha,access='direct',status='replace',recl=2*nbyte*num_mix*num_comps)
    write(11,rec=1) alpha; call flush(11); close(11)
    open(unit=12,file=trim(outdirparam)//'/history/'//trim(adjustl(tmpstring))//'/' &
      //outfile_mu,access='direct',status='replace',recl=2*nbyte*num_mix*num_comps)
    write(12,rec=1) mu; call flush(12); close(12)
    open(unit=13,file=trim(outdirparam)//'/history/'//trim(adjustl(tmpstring))//'/' &
      //outfile_sbeta,access='direct',status='replace',recl=2*nbyte*num_mix*num_comps)
    write(13,rec=1) sbeta; call flush(13); close(13)
    open(unit=14,file=trim(outdirparam)//'/history/'//trim(adjustl(tmpstring))//'/' &
      //outfile_rho,access='direct',status='replace',recl=2*nbyte*num_mix*num_comps)
    write(14,rec=1) rho; call flush(14); close(14)
    open(unit=15,file=trim(outdirparam)//'/history/'//trim(adjustl(tmpstring))//'/' &
      //outfile_mean,access='direct',status='replace',recl=2*nbyte*nx)
    write(15,rec=1) mean; call flush(15); close(15)
    open(unit=16,file=trim(outdirparam)//'/history/'//trim(adjustl(tmpstring))//'/' &
      //outfile_sphere,access='direct',status='replace',recl=2*nbyte*nx*nx)
    write(16,rec=1) S; call flush(16); close(16)
    open(unit=21,file=trim(outdirparam)//'/history/'//trim(adjustl(tmpstring))//'/' &
      //outfile_A,access='direct',status='replace',recl=2*nbyte*nw*num_comps)
    write(21,rec=1) A; call flush(21); close(21)
    open(unit=22,file=trim(outdirparam)//'/history/'//trim(adjustl(tmpstring))//'/' &
      //outfile_comp_list,access='direct',status='replace',recl=2*nbyte*nw*num_models)
...
subroutine write_output
  ...
  if (write_LLt) then
     do j = 1,seg_nodes
        if (j .eq. seg_rank+1) then
           do seg = 1,numsegs
              if (outdirparam == '') then
                 open(unit=19,file=outdir//date(5:8)//'_'//time(1:6)//'/' &
      //outfile_LLt,access='direct',recl=2*nbyte)
              else
  ...
  if (myrank == 0) then
     ...
     if (outdirparam == '') then
        open(unit=29,file=outdir//date(5:8)//'_'//time(1:6)//'/' &
      //outfile_c,access='direct',status='replace',recl=2*nbyte*nw*num_models)
        write(29,rec=1) c; call flush(29); close(29)
        open(unit=9,file=outdir//date(5:8)//'_'//time(1:6)//'/' &
      //outfile_w,access='direct',status='replace',recl=2*nbyte*nw*nw*num_models)
        write(9,rec=1) W; call flush(9); close(9)
        open(unit=10,file=outdir//date(5:8)//'_'//time(1:6)//'/' &
      //outfile_gamma,access='direct',status='replace',recl=2*nbyte*num_models)
        write(10,rec=1) gm; call flush(10); close(10)
        open(unit=11,file=outdir//date(5:8)//'_'//time(1:6)//'/' &
      //outfile_alpha,access='direct',status='replace',recl=2*nbyte*num_mix*num_comps)
        write(11,rec=1) alpha; call flush(11); close(11)
        open(unit=12,file=outdir//date(5:8)//'_'//time(1:6)//'/' &
      //outfile_mu,access='direct',status='replace',recl=2*nbyte*num_mix*num_comps)
        write(12,rec=1) mu; call flush(12); close(12)
        open(unit=13,file=outdir//date(5:8)//'_'//time(1:6)//'/' &
      //outfile_sbeta,access='direct',status='replace',recl=2*nbyte*num_mix*num_comps)
        write(13,rec=1) sbeta; call flush(13); close(13)
        open(unit=14,file=outdir//date(5:8)//'_'//time(1:6)//'/' &
      //outfile_rho,access='direct',status='replace',recl=2*nbyte*num_mix*num_comps)
        write(14,rec=1) rho; call flush(14); close(14)
        open(unit=15,file=outdir//date(5:8)//'_'//time(1:6)//'/' &
      //outfile_mean,access='direct',status='replace',recl=2*nbyte*nx)
        write(15,rec=1) mean; call flush(15); close(15)
        open(unit=16,file=outdir//date(5:8)//'_'//time(1:6)//'/' &
      //outfile_sphere,access='direct',status='replace',recl=2*nbyte*nx*nx)
        write(16,rec=1) S; call flush(16); close(16)
        open(unit=17,file=outdir//date(5:8)//'_'//time(1:6)//'/' &
      //outfile_LL,access='direct',status='replace',recl=2*nbyte*max(1,max_iter))
        write(17,rec=1) LL; call flush(17); close(17)
        if (write_nd) then
           open(unit=18,file=outdir//date(5:8)//'_'//time(1:6)//'/' &
      //outfile_nd,access='direct',status='replace',recl=2*nbyte*max(1,max_iter)*num_comps)
           write(18,rec=1) nd; call flush(18); close(18)            
        end if
        open(unit=21,file=outdir//date(5:8)//'_'//time(1:6)//'/' &
      //outfile_A,access='direct',status='replace',recl=2*nbyte*nw*num_comps)
        write(21,rec=1) A; call flush(21); close(21)
        open(unit=22,file=outdir//date(5:8)//'_'//time(1:6)//'/' &
      //outfile_comp_list,access='direct',status='replace',recl=2*nbyte*nw*num_models)
        write(22,rec=1) comp_list; call flush(21); close(22)
     else
        open(unit=29,file=trim(outdirparam)//'/' &
      //outfile_c,access='direct',status='replace',recl=2*nbyte*nw*num_models)
        write(29,rec=1) c; call flush(29); close(29)
        open(unit=9,file=trim(outdirparam)//'/' &
      //outfile_w,access='direct',status='replace',recl=2*nbyte*nw*nw*num_models)
        write(9,rec=1) W; call flush(9); close(9)
        open(unit=10,file=trim(outdirparam)//'/' &
      //outfile_gamma,access='direct',status='replace',recl=2*nbyte*num_models)
        write(10,rec=1) gm; call flush(10); close(10)
        open(unit=11,file=trim(outdirparam)//'/' &
      //outfile_alpha,access='direct',status='replace',recl=2*nbyte*num_mix*num_comps)
        write(11,rec=1) alpha; call flush(11); close(11)
        open(unit=12,file=trim(outdirparam)//'/' &
      //outfile_mu,access='direct',status='replace',recl=2*nbyte*num_mix*num_comps)
        write(12,rec=1) mu; call flush(12); close(12)
        open(unit=13,file=trim(outdirparam)//'/' &
      //outfile_sbeta,access='direct',status='replace',recl=2*nbyte*num_mix*num_comps)
        write(13,rec=1) sbeta; call flush(13); close(13)
        open(unit=14,file=trim(outdirparam)//'/' &
      //outfile_rho,access='direct',status='replace',recl=2*nbyte*num_mix*num_comps)
        write(14,rec=1) rho; call flush(14); close(14)
        open(unit=15,file=trim(outdirparam)//'/' &
      //outfile_mean,access='direct',status='replace',recl=2*nbyte*nx)
        write(15,rec=1) mean; call flush(15); close(15)
        open(unit=16,file=trim(outdirparam)//'/' &
      //outfile_sphere,access='direct',status='replace',recl=2*nbyte*nx*nx)
        write(16,rec=1) S; call flush(16); close(16)
        open(unit=17,file=trim(outdirparam)//'/' &
      //outfile_LL,access='direct',status='replace',recl=2*nbyte*max(1,max_iter))
        write(17,rec=1) LL; call flush(17); close(17)            
        if (write_nd) then
           open(unit=18,file=trim(outdirparam)//'/' &
      //outfile_nd,access='direct',status='replace',recl=2*nbyte*max(1,max_iter)*num_comps)
           write(18,rec=1) nd; call flush(18); close(18)    
        end if
        open(unit=21,file=trim(outdirparam)//'/' &
      //outfile_A,access='direct',status='replace',recl=2*nbyte*nw*num_comps)
        write(21,rec=1) A; call flush(21); close(21)
        open(unit=22,file=trim(outdirparam)//'/' &
      //outfile_comp_list,access='direct',status='replace',recl=2*nbyte*nw*num_models)
        write(22,rec=1) comp_list; call flush(21); close(22)
     end if
