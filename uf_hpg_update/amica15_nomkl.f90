...
! use mpi
! #endif
! (07/17/2025) JS, updating to modern compiler with openblas and newer openmpi + gcc
...
include 'amica15_header_nomkl.f90'
...
do
   ...
   if (startover) then
      ...
      !----- reject data
      if ((do_reject .and. (maxrej > 0)) .and. ((iter==rejstart) .or. ((mod(max(1,iter-rejstart),rejint) == 0) &
       .and. (numrej < maxrej)))) then
...
! (07/17/2025) JS, updating to use newere compilers
