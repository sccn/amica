...
DOUBLE PRECISION, ALLOCATABLE :: c(:,:), cx(:,:), wc(:,:), dc_numer_tmp_t(:,:,:),&
 dc_numer_tmp(:,:), dc_numer(:,:), dc_denom_tmp_t(:,:,:), dc_denom_tmp(:,:), dc_denom(:,:)
double precision, allocatable :: S(:,:), dCov(:,:), Spinv(:,:), Spinv2(:,:),&
 Stmp(:,:), Stmp2(:,:), Stmp3(:,:), dWtmp_t(:,:,:,:), dWtmp(:,:,:), ipivnx(:), ipivnw(:)
...
DOUBLE PRECISION, ALLOCATABLE :: dalpha_numer_tmp_t(:,:,:), dalpha_denom_tmp_t(:,:,:),&
 dalpha_numer_tmp(:,:), dalpha_denom_tmp(:,:), dalpha_numer(:,:), dalpha_denom(:,:)
DOUBLE PRECISION, ALLOCATABLE :: dmu_numer_tmp_t(:,:,:), dmu_denom_tmp_t(:,:,:),&
 dmu_numer_tmp(:,:), dmu_denom_tmp(:,:), dmu_numer(:,:), dmu_denom(:,:)
DOUBLE PRECISION, ALLOCATABLE :: dbeta_numer_tmp_t(:,:,:), dbeta_denom_tmp_t(:,:,:),&
 dbeta_numer_tmp(:,:), dbeta_denom_tmp(:,:), dbeta_numer(:,:), dbeta_denom(:,:)
double precision, allocatable :: drho_numer_tmp_t(:,:,:), drho_denom_tmp_t(:,:,:),&
 drho_numer_tmp(:,:), drho_denom_tmp(:,:), drho_numer(:,:), drho_denom(:,:)

DOUBLE PRECISION, ALLOCATABLE :: dbaralpha_numer_tmp_t(:,:,:,:), dbaralpha_denom_tmp_t(:,:,:,:),&
 dbaralpha_numer_tmp(:,:,:), dbaralpha_denom_tmp(:,:,:), dbaralpha_numer(:,:,:), dbaralpha_denom(:,:,:)
DOUBLE PRECISION, ALLOCATABLE :: dlambda_numer_tmp_t(:,:,:,:), dlambda_denom_tmp_t(:,:,:,:),&
 dlambda_numer_tmp(:,:,:), dlambda_denom_tmp(:,:,:), dlambda_numer(:,:,:), dlambda_denom(:,:,:)
DOUBLE PRECISION, ALLOCATABLE :: dkappa_numer_tmp_t(:,:,:,:), dkappa_denom_tmp_t(:,:,:,:),&
 dkappa_numer_tmp(:,:,:), dkappa_denom_tmp(:,:,:), dkappa_numer(:,:,:), dkappa_denom(:,:,:)
DOUBLE PRECISION, ALLOCATABLE :: dsigma2_numer_tmp_t(:,:,:), dsigma2_denom_tmp_t(:,:,:),&
 dsigma2_numer_tmp(:,:), dsigma2_denom_tmp(:,:), dsigma2_numer(:,:), dsigma2_denom(:,:)
...
double precision :: mincond = 1.0e-15, minrho = 1.0, maxrho = 2.0,&
 minlog = -1500.0, LLinc, maxdble = 1.0e32, mineigv = 1.0e-15, mineig = 1.0e-15;
double precision :: ndtmpsum, ndtmpsumsum, rejsig = 3.0, LLtmp, LLtmp2, lrate0, pcadb
double precision :: lrate = 0.1, minlrate = 1.0e-12, lratefact = 0.5,&
 rho0 = 1.5, rholrate = 0.05, rholrate0 = 0.05
...
integer :: num_comps = -1, num_mix = 3, num_mix_init = 3, share_iter = 100,&
 share_start = 100, blk_min = 128, blk_max = 1024
